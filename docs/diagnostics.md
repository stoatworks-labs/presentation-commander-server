...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/presentation-commander-server/` |
| Linux | `$XDG_STATE_HOME/presentation-commander-server/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\presentation-commander-server\logs\` |

`PC_SERVER_LOG_DIR` overrides it. The path is printed on the first line of every
...
`presentation-commander-server.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `PC_SERVER_LOG`, default `info`.
...
```bash
presentation-commander-server --collect-diagnostics
```

Writes `presentation-commander-server-diagnostics-<timestamp>.json` and prints its path. One file, so
