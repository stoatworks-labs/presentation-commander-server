# Running unsigned Presentation Commander Server builds
...
Presentation Commander Server listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of Presentation Commander Server**
...
domain. Presentation Commander Server needs this to serve its JSON-RPC control API and route NDI video.

If you deny it (or the prompt appears behind another window and times out), clients, Companion and the automation API will all fail to connect
...
**Private**. If Presentation Commander Server isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "Presentation Commander Server" -Direction Inbound -Program "C:\Path\To\Presentation Commander Server.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*Presentation Commander Server*" | Remove-NetFirewallRule
```
...
```sh
chmod +x ./presentation-commander-server
```
...
```sh
codesign -dv --verbose=4 "/Applications/presentation-commander-server.app"
spctl -a -vv "/Applications/presentation-commander-server.app"   # still reports "rejected" — ad-hoc is not notarization
```
